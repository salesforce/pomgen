...
do
    if ! [[ "$action" =~ ^(clean|pomgen|install|install_all|deploy_all|deploy_only|build|version-check)$ ]]; then
        echo "[ERROR] action [$action] must be one of [clean|pomgen|install|install_all|deploy_all|deploy_only|build|version-check]" && exit 1
    fi
    ...
    elif [ "$action" == "version-check" ]; then
        if [ -z "$REPOSITORY_URL" ]; then
            echo "[ERROR] REPOSITORY_URL must be set"
            exit 1
        fi
        VERSION_INCREMENT_COMPONENT="${VERSION_INCREMENT_COMPONENT:-patch}"
        _for_each_pom "version_check" $repo_root_path $pom_base_filename $jar_artifact_classifier $library_path
