...
`poppy` is a package manager integration layer for Bazel-built artifacts. 
...
Bazel is generally used for building large repositories. In an ideal world, all dependencies and all their consumers exist in the same repository. `poppy` is for the non-ideal world case, where some consumers have to remain outside of the Bazel-built repository. In this scenario, dependencies have to be shared with the outside via a package manager. `poppy` provides the glue between Bazel and the package manager.
